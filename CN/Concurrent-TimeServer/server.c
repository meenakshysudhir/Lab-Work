    ...
    while (1) {
        ...
        if ((pid = fork()) < 0) {
            ...
            if (strcmp(buffer, "time") == 0) {
                ...
                int sock_fd_child = socket(AF_INET, SOCK_DGRAM, 0);  // new socket for child
                if (sock_fd_child < 0) {
                    perror("Child socket error");
                    exit(EXIT_FAILURE);
                }

                if (sendto(sock_fd_child, current_time, strlen(current_time), 0, (struct sockaddr*)&cli_addr, cli_len) < 0) {
